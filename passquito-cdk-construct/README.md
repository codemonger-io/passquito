...
It has a special version number followed by a dash (`-`) plus a short commit hash; e.g., `0.0.5-abc1234` where `abc1234` is the short commit hash (the first 7 characters) of the commit used to build the package (_snapshot_).
...
```sh
npm install @codemonger-io/passquito-cdk-construct@0.0.5-abc1234
```

Please replace `0.0.5-abc1234` with the actual version number of the _snapshot_ you want to install, which is available in the [package repository](https://github.com/codemonger-io/passquito/pkgs/npm/passquito-cdk-construct).
